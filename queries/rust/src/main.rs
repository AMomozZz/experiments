...
const GUEST_RS_WASI3_MODULE: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../wasip3-adapter/target/wasm32-wasip1/debug/guest.component.wasm"
));
...
fn main() {
    ...
    let wi: WasiImpl<Host> = WasiImpl(IoImpl::<Host>(host));
    let store_wrapper = Rc::new(RefCell::new(Store::new(&engine, wi)));

    let mut linker= Linker::new(&engine);
    // linker.root().;
    add_to_linker_async::<WasiImpl<Host>>(&mut linker).unwrap();
    ...
    let wasm_func_stream_q1 = WasmFunction::<(HostStream<String>,), (HostStream<String>,)>::new(&linker, &engine, GUEST_RS_WASI3_MODULE, &store_wrapper, "pkg:component/nexmark", "q1");
    ...
    match query.as_str() {
        ...
        // wasm
        "q1-wasm-stream" => timed(move |ctx| q1::run_wasm_stream(stream(ctx, bids), ctx, wasm_func_stream_q1)),
